...
world_region_df_index.drop(['Caribbean', 'Central Europe, Eastern Europe, and Central Asia', 'England',
       ...
       'Low-middle SDI', 'Middle SDI', 'Wales'], inplace=True)
...
if manipulated_data:
    ...
    st.write('dataframe containing rows only for individual countries:')
    st.write(air_poll_df)
    st.write('dataframe containing rows only for world regions:')
...
with col_2:
    ...
    ax.set_ylabel('Air Pollution (deaths per 100k)')
    st.pyplot(fig)
st.write('In all cases, we see a clear reduction in worldwide deaths due to air polution.')
